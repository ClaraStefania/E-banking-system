...
public class ChangeDepositLimit implements Command {
    ...
    public ChangeDepositLimit(final CommandInput command, final List<User> users,
    ...
    /**
     * changes the deposit limit for the business account
     */
    @Override
    public void execute() {
        ...
        if (user.getEmail().equals(accountOwnerEmail)) {
            ((BusinessAccount) account).setDepositLimit(command.getAmount());
        } else {
            ObjectNode rootNode = objectMapper.createObjectNode();
            rootNode.put("command", Constants.CHANGE_DEPOSIT_LIMIT);
            ObjectNode errorNode = objectMapper.createObjectNode();
            errorNode.put("timestamp", command.getTimestamp());
            errorNode.put("description", "You must be owner in order to"
